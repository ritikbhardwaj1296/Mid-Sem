...
if __name__ == "__main__":
    ...
    #hit_point = []  # x, y  hit point which will be used to store the x and y coordinates of where the robot has been hit with an obstacle.
    #leave_point = []  # x, y
    while robot.step(TIME_STEP) != -1:
        ...
        if state == 'start':
            ...
            print("front_sonar",front_sonar)
            
            if(is_on_M_line(gps_values[0], gps_values[1])):
                print("robot is on M line")
                ...
                print("far_from_wall_counter",far_from_wall_counter)
            ...
            if left_sonar < 200 or left_ir_values[0] < 300 :
                print("left_ir_values",left_ir_values[0])
                print("right_ir_values[0]",right_ir_values[0])
                prev = state
                state = 'turn'
                theta_0 = get_bearing_in_degrees(compass_val)
            ...
            prev = state
            state = 'wall_following'        
            # is_aligned = align_to_M(get_bearing_in_degrees(compass_val), theta=theta_0+90, turn_left=False)
            # if is_aligned:
                # print("is aligned")
                # prev = state
                # state = 'wall_following'
